...
class AdvancedLoginHistoryDashboardController(http.Controller):
    ...
    def _format_datetime(self, datetime_value):
        ...
        return format_datetime(request.env, datetime_value, dt_format=False)
    ...
    def _prepare_login_row(self, record):
        status_label = _("Successful Login")
        status_class = "success"

        if record.status == "logout":
            status_label = _("Logout")
            status_class = "logout"
    ...
    def get_dashboard_data(self):
        if not request.env.user.has_group("mst_advanced_login_history.group_ickab_audit_manager"):
            raise AccessError(_("You do not have permission to view ICKAB Audit History."))
