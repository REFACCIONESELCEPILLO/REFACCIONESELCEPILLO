        ...
        if self.action_type == "delete":
            return {
                ...
                "params": {
                    ...
                    "message": _("This record was deleted, so it cannot be opened."),
                    "type": "warning",
                    "sticky": False,
                },
            }
        ...
        if self.model_name not in self.env:
            return {
                ...
                "params": {
                    ...
                    "message": _("The related model is not available in this database."),
                    "type": "warning",
                    "sticky": False,
                },
            }
        ...
        if not record.exists():
            return {
                ...
                "params": {
                    ...
                    "message": _("The original record is no longer available."),
                    "type": "warning",
                    "sticky": False,
                },
            }
...
class BaseModelActivityAudit(models.AbstractModel):
    ...
    def _audit_should_skip(self, action_type=False):
        ...
        # ICKAB: Model Log Configuration is the real audit scope.
        # Unconfigured models and disabled operations must not generate logs.
        if action_type and not self._audit_get_model_config(self._name, action_type):
            return True
    ...
    def _audit_create_log(self, record, action_type, line_commands):
        try:
            ...
            # ICKAB: do not create Advanced Logs for models/actions that are
            # outside an active Model Log Configuration.
            if not model_config:
                return True
    ...
    def create(self, vals_list):
        if self._audit_should_skip("create"):
    ...
    def write(self, vals):
        if self._audit_should_skip("modify"):
    ...
    def unlink(self):
        if self._audit_should_skip("delete"):
            return super().unlink()
