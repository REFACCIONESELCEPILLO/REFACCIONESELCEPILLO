...
class ResUsers(models.Model):
            ...
            try:
                if request and request.env:
                    request.env['login.failed.history'].sudo().create_failed_login_record(
                        login=login,
                        reason=_('Invalid login credentials')
                    )
    ...
    def action_kill_all_sessions(self):
        if not self.env.user.has_group("mst_advanced_login_history.group_ickab_audit_manager"):
            raise AccessError(_("You do not have permission to manage audit sessions."))
        ...
        return {
            ...
            "params": {
                "title": _("Sessions Killed"),
                "message": _("All active sessions for this user have been killed."),
                "type": "success",
                "sticky": False,
            },
        }