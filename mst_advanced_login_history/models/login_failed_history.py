...
class LoginFailedHistory(models.Model):
    ...
    def _compute_display_name(self):
        for record in self:
            login_name = record.login or _("Unknown Login")
            ...
            if record.attempt_time:
                ...
                record.display_name = _("%s - Failed Login - %s") % (
                    login_name,
                    attempt_time
                )
            else:
                record.display_name = _("%s - Failed Login") % login_name
    ...
    def create_failed_login_record(
        ...
        return self.sudo().create({
            ...
            'reason': reason or _('Invalid login credentials'),
        })
