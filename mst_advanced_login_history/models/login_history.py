...
class LoginHistory(models.Model):
    ...
    def _compute_display_name(self):
        for record in self:
            user_name = record.user_id.name or _("Unknown User")
    ...
    def create_login_record(self, user):
        ...
        # ICKAB: keep simultaneous sessions independent. A new login must not
        # mark other still-valid sessions for the same user as logged out.
    ...
    def logout_user_record(self, user_id=False, session_id=False):
        ...
        if current_session_id:
            ...
            # Fallback only when no session id exists. If a session id was
            # supplied but no record matches, never close another session.
            record = self.sudo().search([
